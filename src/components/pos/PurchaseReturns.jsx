  ...
  const mappedReturns = React.useMemo(() => {
    ...
    return purchaseReturns.map((ret, index) => {
      const vendorId = String(
        getFirstDefined(ret?.vendor?.id, ret?.vendor_id, ret?.vendor, ""),
      ).trim();
      ...
      const paidStatus = getFirstDefined(
        ret?.paid_status,
        ret?.payment_status,
        ret?.status_label,
        "Open",
      ...
      return {
        key: getFirstDefined(
          ret?.id,
          ret?.uuid,
          ret?.return_bill_number,
          index,
        ),
        billNo: getFirstDefined(
          ret?.return_bill_number,
          ret?.return_number,
          ret?.bill_id,
          `RET-${ret?.id ?? index + 1}`,
        ...
        dueDate: formatDate(
          getFirstDefined(ret?.due_date, ret?.payment_due_date),
        ),
        paidStatus,
  ...
  const filteredReturns = React.useMemo(() => {
    ...
    return mappedReturns.filter((record) => {
      if (filterPeriod !== "All") {
        ...
        if (
          record.returnDateValue < periodRange.rangeStart ||
          record.returnDateValue > periodRange.rangeEnd
        )
          return false;
      }
      ...
      if (filterBy === "SKU/UPC") {
        matchesFilterBy =
          ...
          record.skuUpcText.includes(normalizedFilterValue);
      } else if (filterBy === "Product Code") {
        matchesFilterBy =
          !normalizedFilterValue ||
          record.productCodeText.includes(normalizedFilterValue);
      } else if (filterBy === "Vendor") {
        matchesFilterBy =
          !selectedVendorFilter || record.vendor === selectedVendorFilter;
      } else if (filterBy === "Status") {
        matchesFilterBy =
          !selectedStatusFilter || record.status === selectedStatusFilter;
