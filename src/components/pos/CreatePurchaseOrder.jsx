...
const getNextPoNumber = (ordersData) => {
  ...
  const maxPoNumber = orders.reduce((max, order) => {
    const rawPoNumber = getFirstDefined(
      order?.po_number,
      order?.po_no,
      order?.number,
      order?.order_number,
...
const CreatePurchaseOrder = () => {
  ...
  const {
    ...
    error: productsError,
  } = useFetch("/inventory/products/");
  const { data: ordersData, loading: ordersLoading } = useFetch(
    "/purchasing/orders/",
  );
  const { data: poData, loading: poLoading } = useFetch(
  ...
  const vendors = useMemo(() => {
    ...
    if (Array.isArray(vendorsData?.data?.results))
      return vendorsData.data.results;
  ...
  const vendorOptions = useMemo(() => {
    return vendors.map((v) => ({
      ...
      salesPerson:
        v.sales_person || v.contact_name || v.contact_person || "N/A",
    }));
  }, [vendors]);
  ...
  const selectedVendorLabel = useMemo(() => {
    return (
      vendorOptions.find((v) => String(v.id) === String(selectedVendor))
        ?.name || ""
    );
  ...
  const products = useMemo(() => {
    ...
    return productList.map((product, index) => {
      const size = getFirstDefined(
        product.size?.name,
        product.size_name,
        product.size,
        "",
      );
      const pack = getFirstDefined(
        product.pack?.name,
        product.pack_name,
        product.pack,
        "",
      ...
      return {
        ...
        sizePack: sizePack || "-",
        stock: asNumber(
          getFirstDefined(product.stock_quantity, product.quantity, 0),
        ),
        unitPrice,
        buyAsBottle: Boolean(
          getFirstDefined(product.buy_as_bottle, product.buy_as_unit, false),
        ),
      };
    });
  }, [productsData]);
  ...
  const filteredProductOptions = useMemo(() => {
    ...
    return productOptions.filter(
      (p) =>
        p.name.toLowerCase().includes(q) || p.subText.toLowerCase().includes(q),
    );
  }, [productOptions, productSearch]);

  const selectedProductLabel = useMemo(() => {
    return (
      productOptions.find((p) => String(p.id) === String(selectedProductId))
        ?.name || ""
    );
  ...
  useEffect(() => {
    if (poData) {
      ...
      const vendorIdValue = String(
        getFirstDefined(poData.vendor?.id, poData.vendor_id, poData.vendor, ""),
      );
      setSelectedVendor(vendorIdValue);

      setVendorOrderNumber(
        getFirstDefined(
          poData.vendor_order_ref,
          poData.vendor_order_number,
          poData.vendor_order_no,
          "",
        ),
      );
      ...
      const displayAddress =
        poData.address ||
        (poData.vendor_details?.address_details
          ? [
              poData.vendor_details.address_details.address_1,
              poData.vendor_details.address_details.address_2,
              poData.vendor_details.address_details.city,
            ]
              .filter(Boolean)
              .join(", ")
          : "");
      ...
      const statusValue = String(
        getFirstDefined(poData.status, poData.po_status, ""),
      ).toLowerCase();
      const isReceived =
        statusValue.includes("fully received") || statusValue === "closed";
      setIsReadOnly(isReceived);

      const receiveDay = getFirstDefined(
        poData.received_date,
        poData.actual_delivery_date,
        poData.received_at,
        isReceived ? poData.updated_at : "",
      );
      setReceivedDate(toInputDate(receiveDay));

      if (Array.isArray(poData.items)) {
        const mappedItems = poData.items.map((item, idx) => {
          const productIdValue = getFirstDefined(
            item.product?.id,
            item.product_id,
            item.product,
          );
          ...
          const fullProduct =
            products.find((p) => String(p.id) === String(productIdValue)) ||
            item.product ||
            {};

          const caseUnits = asNumber(
            getFirstDefined(
              fullProduct.caseUnits,
              fullProduct.case_units,
              fullProduct.bpc,
              1,
            ),
            1,
          ...
          return {
            ...
            vendorCode: getFirstDefined(
              fullProduct.vendorCode,
              fullProduct.vendor_code,
              "",
            ),
            itemName: getFirstDefined(
              fullProduct.itemName,
              fullProduct.name,
            ...
            sizePack: getFirstDefined(
              fullProduct.sizePack,
              [fullProduct.size_name, fullProduct.pack_name]
                .filter(Boolean)
                .join(",") || "-",
            ),
            buyAsCase: false,
  ...
  const handleProductCodeSubmit = () => {
    const normalizedCode = String(productCodeInput || "")
      .trim()
      .toLowerCase();
    if (!normalizedCode) {
      setProductCodeLookupMessage("");
  ...
  const selectedCaseUnits = Math.max(
    asNumber(selectedProduct?.caseUnits, 1),
    1,
  );
  const selectedCaseCost =
    asNumber(addUnitCost, selectedProduct?.lastCost) * selectedCaseUnits;

  const totalAmount = useMemo(
    () =>
      items.reduce(
        (acc, item) => acc + asNumber(item.qty) * asNumber(item.unitCost),
        0,
      ),
    [items],
  );
    ...
    try {
      const payload = {
        ...
        items: items.map((item) => ({
          ...
          quantity_received: 0,
          unit_price: asNumber(
            getFirstDefined(item.unitCost, item.lastCost, 0),
          ).toFixed(2),
        })),
      };
  ...
  return (
    <div className="h-full min-h-0 flex flex-col">
      <div className="flex-1 min-h-0 overflow-y-auto pr-1 pb-24 space-y-6">
            ...
            <div className="relative w-full max-w-[560px] rounded-md border border-slate-300 bg-white p-6 shadow-2xl">
              <div className="min-h-[170px] flex items-center justify-center text-center px-4">
                <p className="text-[18px] leading-relaxed font-medium text-slate-900">
                  Item with sku '{duplicateSku}' already exist. please add
                  another item.
                </p>
              </div>
              <button
        ...
        <Card
          noPadding
          className="border-slate-200 shadow-sm mb-6 !rounded-lg overflow-visible"
        >
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-semibold text-slate-500 ml-0.5">
                  Select Vendor
                </label>
        ...
        {!isReadOnly && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card
              noPadding
              className="lg:col-span-2 border-slate-200 shadow-sm !rounded-lg overflow-visible"
            >
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3">
                ...
                <div className="flex flex-col gap-1.5 lg:col-span-3">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider font-poppins text-sky-600">
                    Quantity
                  </label>
                  <input
                    type="number"
              ...
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="space-y-4 pt-6 mt-6 border-t border-slate-100">
                  ...
                  <div className="flex justify-between items-center bg-[#f0f9ff]/50 p-4 rounded-xl border border-sky-100">
                    <h3 className="text-[16px] font-black text-slate-800 tracking-tight font-poppins">
                      Total
                    </h3>
          ...
          <div className="flex justify-end pt-4">
            <Card className="w-full lg:max-w-md border-emerald-100 bg-emerald-50/10 !rounded-xl">
              <div className="flex justify-between items-center text-emerald-800">
                ...
                <span className="text-xs font-black px-4 py-1.5 bg-emerald-100 rounded-full">
                  {poData?.status}
                </span>
              </div>
            </Card>
            ...
            <table className="w-full text-left border-collapse table-auto">
              <thead className="bg-[#f9fafb] border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    Vendor Code
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    Item Name
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right">
                    Last Cost
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">
                    Case Units
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">
                    Quantity
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right whitespace-nowrap">
                    Line Total
                  </th>
                  <th className="px-3 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    Size/Pack
                ...
                {productsLoading ? (
                  ...
                  items.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 text-[13px] font-medium text-slate-500">
                        {item.vendorCode || "-"}
                      </td>
                      <td className="px-3 py-3 text-[14px] font-bold text-slate-700">
                        {item.itemName}
                      </td>
                        ...
                        {(item.buyAsCase
                          ...
                          : asNumber(
                              getFirstDefined(
                                ...
                                ) * asNumber(item.caseUnits, 1),
                              ),
                            )
                        ).toFixed(2)}
                      </td>
                      <td className="px-3 py-3 text-[14px] font-semibold text-slate-700 text-center">
                        {asNumber(item.caseUnits).toFixed(0)}
                      </td>
                      <td className="px-3 py-3 text-[14px] font-semibold text-slate-700 text-center">
                        {formatCompactNumber(item.qty, "0")}
                      </td>
                ...
                {filteredVendorOptions.length === 0 ? (
                  ...
                  filteredVendorOptions.map((vendor) => (
                    ...
                    >
                      ...
                      <div className="text-xs font-bold text-slate-400">
                        Sales Person: {vendor.salesPerson || "-"}
                      </div>
                    </button>
                  ))
                )}
