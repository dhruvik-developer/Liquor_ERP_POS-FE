...
const UserTable = ({
  ...
  <SectionCard
    title="User Overview"
    subtitle="System access control and registry management"
  >
      ...
      <table className="min-w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-neutral-50">
          {users.map((user) => (
            ...
            >
              <td className="px-6 py-5 text-sm font-black text-neutral-800 group-hover:text-primary transition-colors">
                {user.name}
              </td>
              ...
              <td className="px-6 py-5">
                <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 bg-neutral-100 rounded-md text-neutral-600 border border-neutral-200/50">
                  {user.roleName}
                </span>
              </td>
              <td className="px-6 py-5 text-sm font-bold text-neutral-500 italic">
                {user.storeNames.length
                  ? user.storeNames.join(", ")
                  : "No Stores Assigned"}
              </td>
              <td className="px-6 py-5 text-center">
                <span
                  className={`inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-widest border ${statusClass(user.isActive)}`}
                >
    ...
    <div className="mt-8 flex items-center justify-between px-2">
      <p className="text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400">
        Page {pagination.page} <span className="text-neutral-200 mx-2">/</span>{" "}
        {Math.max(1, Math.ceil((pagination.total || 1) / pagination.pageSize))}
      </p>
